...
@section("content")
        <div class="col-sm-8">
            <blockquote>
                <p>{{$topic->name}}</p>
                <footer>文章：{{$topic->post_topics_count}}</footer>    <!-- 文章數-->

                                                        <!--從data-toggle  到data-target  是讓按鈕點下去彈出小視窗-->
                <button class="btn btn-defalut topic-submit" data-toggle='modal' data-target="#topic_submit_modal" topic-id="{{$topic->id}}" type="button">投稿</button>
            </blockquote>
        </div>
